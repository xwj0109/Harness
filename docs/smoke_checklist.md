...
- The TUI layout keeps chat and dashboard context in stable read-only sections, shows keyboard/navigation hints, reports no-match states, and displays only static ASCII cat art without persisting preferences, loading external assets, mutating harness state, or adding command actions.
