...
- `harness tui` is read-only and optional; without the TUI extra it returns a stable install hint. With the extra installed it renders project state, summary counts, imported agents, task details, active lease details, daemon event summaries, recent runs, safety reminders, local in-memory search over loaded panes, and a copy-only command palette without initializing projects, importing agents, creating tasks, creating runs, creating artifacts, acquiring leases, mutating daemon state, executing adapters, crawling files, or searching artifact contents.
- The v1.7 command palette shows grouped command templates, mutation/safety notes, and selected command text for manual use only. It must not execute commands, spawn subprocesses, invoke a shell, copy to the clipboard, run daemon actions, execute adapters, preflight backends, run Docker, call providers, or expose artifact file contents.
- The v1.8 TUI layout groups panes into stable read-only sections, shows keyboard/navigation hints, and reports no-match states without persisting preferences, mutating harness state, or adding command actions.
