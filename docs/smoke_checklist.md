...
- `harness tui` is read-only and optional; without the TUI extra it returns a stable install hint. With the extra installed it renders project state, summary counts, imported agents, task details, active lease details, daemon event summaries, recent runs, safety reminders, and command suggestions without initializing projects, importing agents, creating tasks, creating runs, creating artifacts, acquiring leases, mutating daemon state, or executing adapters.
