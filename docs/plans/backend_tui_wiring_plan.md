...
- Slice 5 event-driven refresh started: the TUI now starts process-local global/session event subscriptions, swaps the selected-session stream on navigation with replay, marks projections dirty from events, and falls back to low-frequency polling with compact status on stream loss.
- Slice 6 permission cards started: `HarnessAppService.reply_permission()` resolves session permissions and notifies runtime state, non-resume server permission replies route through the service, and the TUI exposes an explicit permission card where `a` allows once, `d` denies, `c` cancels, and Enter does not approve.
- Slice 7 transcript source-of-truth started: `build_tui_transcript_projection()` now renders active-session chat from persisted messages/events with local notices as migration shims, de-duplicates final assistant text from streaming deltas, and redacts secret-like text/artifact bodies.
- Slice 8 attached server mode started: `HarnessHTTPAppService` can attach to an existing bearer-auth local server, read TUI dashboard/session pane/session/message projections over HTTP, consume live SSE session/global event streams, and launch the TUI through explicit `--server-url`/`--token` flags while embedded service remains the default.
- Slice 9 operator docs/smoke checklist started: `docs/operator_guide.md` now documents service-backed embedded TUI mode, explicit attached-server launch, token fail-closed behavior, persisted transcript/permission safety boundaries, and the non-interactive/manual smoke workflow.
- Slice 10 attached TUI acceptance automation started: `tests/test_tui_backend_wiring.py` now runs the Textual app against a real `harness serve` HTTP service client, creates a session through the attached service path, verifies session-pane selection, and confirms live session SSE events invalidate the TUI projection.
- Slice 11 visible runtime failure/default provider started: `HarnessAppService` now wires the configured default provider adapter into embedded runtime prompts, and runtime `model.failed` / failed `harness.turn.finished` events render in the TUI transcript, including the no-provider/no-hidden-fallback failure that previously made simple prompts appear to do nothing.
