...
Without the optional TUI extra, `harness tui` returns a clear install hint instead of importing Textual during normal CLI startup. `harness tui --output json` is a non-interactive availability probe and does not launch a terminal UI. With the optional extra installed, the TUI surface renders a chat-style interface with a slash-command prompt, project state, summary counts, imported-agent details, task details, active lease details, daemon event summaries, recent runs, safety reminders, and local in-memory search over already-loaded dashboard and command metadata.
